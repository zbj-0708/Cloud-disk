...
IMToolList::IMToolList(const QString &title  , QWidget *parent) :
...
//    label->setStyleSheet("background-color: #1f2021;");
    ...
    connect(m_label,&QToolButton::clicked , this, &IMToolList::on_ToolButton_Clicked);
}

// 添加
void IMToolList::addItem(QWidget *item)
{
...
QString IMToolList::getWindowText()
{
    return m_label->text();
}
void IMToolList::SetWindowText(QString str)
{
    m_label->setText(str);
}

//删除
void IMToolList::removeItem(QWidget *item)
{
    item->setVisible(false);
...
// 设置标题
void IMToolList::setTile(const QString & title)
{
    m_label->setText(title);
}

// 获取成员数量
int IMToolList::getSize()
{
    return m_itemList.size();
}
void IMToolList::on_ToolButton_Clicked()
{
...
// 鼠标点击事件
void IMToolList:: mousePressEvent(QMouseEvent *event)
{
...
void IMToolList::SearchItem(QString str)
{
    for (int i=0; i<m_itemList.size(); ++i)
    {
        if(  ((IMToolItem*)m_itemList[i] )->GetName().contains( str) )
...
Description: 右键点击按键
*************************************************/
void IMToolList::onClickRightButton()
{
    return;
    if (m_menu == NULL)
    {
